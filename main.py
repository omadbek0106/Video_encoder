...
async def progress(current, total, message, start, prefix):
    if total == 0:
        return
...
async def auto_delete(*paths, delay=3600):
...
async def encode(input_path, output_path, preset, message):
    cmd = f"ffmpeg -y -i '{input_path}' {preset} '{output_path}'"
    process = await asyncio.create_subprocess_shell(cmd)

    while process.returncode is None:
        try:
            await message.edit("🎬 Encode qilinmoqda...")
        except:
            pass
...
async def do_encode(client, query):
    msg = client.storage
    start = time.time()
